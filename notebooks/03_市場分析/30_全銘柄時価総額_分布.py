...
def _():
    ...
    latest_market_cap_df = (
        ...
        .with_columns(
            (pl.col("MarketCap") / 100_000_000).alias("MarketCap_Okuyen"),
        )  # 億円単位に変換
    )
...
def _(latest_market_cap_df):
    ...
    chart_prob = (
        ...
        .encode(
            x=alt.X("MarketCap_Okuyen:Q").title("時価総額（億円）").scale(type="log"),
            y=alt.Y("z_score:Q").title("Z-score (累積確率)"),
            tooltip=[
                alt.Tooltip("Code"),
