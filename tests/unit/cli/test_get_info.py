...
def test_get_info_with_code(mock_get_info: AsyncMock) -> None:
    ...
    mock_get_info.assert_awaited_once_with(MOCK_CODE, None)


def test_get_info_with_date(mock_get_info: AsyncMock) -> None:
    result = runner.invoke(app, ["get", "info", MOCK_DATE])

    assert result.exit_code == 0
    assert str(MOCK_DF) in result.stdout

    mock_get_info.assert_awaited_once_with(None, MOCK_DATE_OBJ)
